    ...
    def update(self):
        ...
        for customer in self.current_customer_list:
            ...
            if customer!=self.depot_location: 
                ...
                # KEVIN
                customer[self.index_dist_to_truck]=m.sqrt((customer[self.index_pos_rel_truck][0])**2+(customer[self.index_pos_rel_truck][1])**2)/self.max_dist
                customer[self.index_dist_to_depot]=m.sqrt((customer[self.index_pos_rel_depot][0])**2+(customer[self.index_pos_rel_depot][1])**2)/self.max_dist
