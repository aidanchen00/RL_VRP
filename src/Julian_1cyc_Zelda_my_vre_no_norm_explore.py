...
def do_explore(cand_indices, cand_dist):
    global my_steps_done
    global global_eps_threshold

    DEBUG_SELECT= False
    sample = r.random()
    eps_threshold = EPS_END + (EPS_START - EPS_END) * \
        m.exp(-1. * my_steps_done / EPS_DECAY)
    my_steps_done += 1
    best_index = -1
    if sample > eps_threshold:
        best_dist  = 1e10
        for idx in range(len(cand_indices)):
            if cand_dist[idx] < best_dist:
                best_dist = cand_dist[idx]
                best_index = idx
        best_index = cand_indices[best_index]
    else:
        best_index = cand_indices[r.randint(0,len(cand_indices)-1)]
    return best_index
...
#        print(f"Original current index: {current_index}")
        if action==self.action_left:
            ...
            #print(f"action: {str(action)}")
            #Shifts the list
#            print(f"!!!! Position_list: {self.position_list}")
            self.position_list[current_index]=0
            self.num_last_moves+=1
            if self.num_last_moves>=len(self.unvisited):
                cand_dist=[]
                for unvisited in self.unvisited:
                    cand_dist.append(self.current_customer_list[unvisited][self.index_dist_to_truck])
                best_index = do_explore(self.unvisited, cand_dist)
                current_index = best_index
                self.position_list[current_index]=1
#                print(f"visited: {self.visited}")
#                print(f"Unvisited: {self.unvisited}")
#                print(f"current_index: {current_index}, best_dist: {best_dist}")
